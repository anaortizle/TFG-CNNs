...
directorio_ruido_Cloudy = 'Saarbrücken_Ruido_Test_Cloudy/';
directorio_Training = 'Saarbrücken_Pano_Training_Cloudy/';

todas_imagesRuido = dir(strcat(directorio_ruido_Cloudy,'*.jpeg'));
...
fila = 1; columnas = 8;
exp4matriz_parametrosCNNresnet50Cloudy = zeros(fila,columnas);
capa = 3;

canal = best_channel(capa);
descriptor_tr_resnet50 = descriptor_training_resnet50(layer_name{capa},canal);

for i = 1:size(todas_imagesRuido,1)
       descriptor_C = [];
       image = imread(sprintf('%s%s',directorio_ruido_Cloudy,todas_imagesRuido(i).name));
...
exp4matriz_parametrosCNNresnet50Cloudy(fila,:) = [capa,canal,e_m_dConvTC,e_v_dConvTC,t_m_dConvTC,t_v_dConvTC,t_est_m_dConvTC,t_est_v_dConvTC];
save(sprintf('%sExp4MatrizResultadosCNNresnet50_dConvC.mat',directorio_ruido_Cloudy),'exp4matriz_parametrosCNNresnet50Cloudy');


% Obtención de los resultados mínimos en el mejor canal de la capa 'conv4_1'
canal_Conv1TC = exp4matriz_parametrosCNNresnet50Cloudy(1,2);
e_m_Conv1TC = exp4matriz_parametrosCNNresnet50Cloudy(1,3);
e_v_Conv1TC = exp4matriz_parametrosCNNresnet50Cloudy(1,4);
t_m_Conv1TC = exp4matriz_parametrosCNNresnet50Cloudy(1,5);
t_v_Conv1TC = exp4matriz_parametrosCNNresnet50Cloudy(1,6);
t_est_m_Conv1TC = exp4matriz_parametrosCNNresnet50Cloudy(1,7);
t_est_v_Conv1TC = exp4matriz_parametrosCNNresnet50Cloudy(1,8);
